...
async function connectToNear() {
  ...
  if (nearConfig.contractName.endsWith('undefined') || nearConfig.contractName.substr(nearConfig.contractName.length -1) === '.') {
    console.warn('Please make sure you have set environment variables REACT_APP_CONTRACT_NAME and REACT_APP_CLIENT_PRIVATE_KEY')
    return null;
  } else {
    const keyStore = new keyStores.InMemoryKeyStore()
    const keyPair = KeyPair.fromString(process.env.REACT_APP_CLIENT_PRIVATE_KEY)
